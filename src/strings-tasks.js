...
function removeFirstOccurrences(str, value) {
  ...
  return index === -1
    ...
    : str
        .substring(0, index)
        .concat(str.substring(index + getStringLength(value)));
}
...
function formatTime(minutes, seconds) {
  ...
  return padValue(String(minutes)).concat(':', padValue(String(seconds)));
}
...
function isPalindrome(str) {
  const cleanStr = (s) => s.replace(/\W/g, '');
  const toLower = (s) => s.toLowerCase();
  return toLower(cleanStr(str)) === toLower(reverseString(cleanStr(str)));
}
...
function reverseWords(str) {
  return str.split(/\s+/).map(reverseString).join(' ');
}
...
function invertCase(str) {
  ...
  return str.split('').map(handleChar).join('');
}
...
function encodeToRot13(str) {
  ...
  const charToRot13 = (char, dictionary) => {
    return dictionary.get(char) || char;
  };

  return str
    .split('')
    .reduce((res, char) => res + charToRot13(char, rot13eng), '');
}
